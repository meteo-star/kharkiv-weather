...
> **Текущая версия:** v1.60.0 (морось видна везде: пороги «шанса» 0.1 мм в часе / 0.2 мм в дне, полоса графика по КОЛИЧЕСТВУ (pmmPoss/pmmMax ≥ 0.1), а не по вероятности; `buildTodayOutlook` сверяется с `nowcastInfo()` и вместо «осадков не ожидается» пишет `outlook.maybePrecip`. Причина: nowcast берёт max по моделям, сводка — их консенсус, при мороси одной модели плашки противоречили. v1.59.0 ниже — «Сегодня» = остаток суток)
