...
> **Текущая версия:** v1.58.0 (согласование осадков по всем поверхностям: единый порог вероятности `PRECIP_POP_SHOW=30%` + 3 состояния дождь/шанс/сухо в посуточном списке и почасовке модалки дня; фактически сухие дни 6%/0мм больше не метятся «дождём»; полоса «возможно» на графике — только при значимом шансе. v1.57.0 ниже — сама полоса из ENS)
