...
> **Текущая версия:** v1.61.0 (**Smart по осадкам**: `p` часа = max(ensP, доля моделей с pmm≥0.1) — ансамбль это только ECMWF, и при его сухом сценарии Smart терял риск целиком (0–2% против 2.1 мм у UKMO); в `dayPrecipView` дневная `precipSum` распределяется по профилю `possMm`, т.к. сумма почасовых медиан систематически 0 (медиана часа ≠ медиана суточных сумм); `buildTodayOutlook` называет возможное количество (`outlook.maybeFrom`). v1.60.0 ниже — морось, v1.59.0 — остаток суток)
> **Состояние:** план «максимальной точности» (этапы 1–4) завершён и в проде; UI-апгрейд V1 (v1.56.0) завершён и в проде; проект в фазе накопления accuracy-данных — см. раздел 7
...
- `renderDays(forecast)` — дни вертикальным списком строк (v1.56.0); `dayTempColor(tc)` — градиент по T для температурной полосы (общая шкала)
- `renderModalHourlyRow(d)` — почасовка в модалке дня вертикальным списком `.mh-*` (v1.56.0)
- `setupBottomDock()` — плавающий нижний док: scrollspy + клик-скролл + закрепление поверх контентных модалок (v1.56.0)
...
| v1.56.0 | **UI-апгрейд V1 (3 этапа из PLAN_UI_UPGRADE.md, одна ветка `feature/day-rows`, мерж после всех этапов).** (1) Дни вертикальным списком строк (`renderDays`→`.day`/`.dr-*`): температурная полоса на ОБЩЕЙ шкале min/max всех дней (`dayTempColor`-градиент), маркер текущей t° на «Сегодня», полоска согласия `.dr-conf`, осадки %+мм, клик→`openModal`. (2) Почасовка в модалке дня вертикальным списком (`renderModalHourlyRow`→`.mh-*`): мини-полоса позиции t° в диапазоне дня, осадки+интенсивность, ветер; прошедшие часы приглушены, текущий — неон; убраны табы метрик+стрелки/swipe; сохранена Smart-строка ансамбля. (3) Плавающий нижний док (`setupBottomDock`/`#bottomDock`): 4 якоря Сейчас/Часы/Дни/Ещё, `dock.*`×16, неон активного, scrollspy (rAF-линия), клик→`window.scrollTo`+коррекция под sticky-шапку, закреплён поверх контентных модалок (день/ветер/осадки/давление/влажность, z-1300, `body.dock-modal-pinned`)/скрыт над утилитарными, стекло прозрачное без заливки `blur 4px`@supports, `html{overflow-anchor:none}`. + fix стрелок графика осадков (`renderPrecipChart`→`updateScrollArrows` после ширины). **Уроки:** backdrop-filter не рендерится в headless-превью (проверять стекло на реале); Chart.js CDN в превью не грузится; `position:fixed`+скролл — `window.scrollTo`, не `scrollIntoView` (body overflow-y:auto). SW v1.56.0-ui-upgrade. |
