...
> **Текущая версия:** v1.51.0 (этап 1 «Честные осадки» — occurrence+amount метрики vs реальные мм ERA5)
...
| 2. Режим «🎯 Smart» | per-variable веса (`computeEnsembleWeights(modelIds, field)`: Tmax/Tmin/precip — отдельные наборы) с заострением `w = 1/(MAE+ε)²` + shrinkage; взвешенная **медиана** для мм-полей (mean размазывает ливни); Smart как отдельный источник рядом с AVG, пишется в accuracy-записи (сайт+сервер) и доказывает себя в таблице | ⬜ следующий |
| 3. Ансамблевая вероятность дождя | Open-Meteo **Ensemble API** (ensemble-api.open-meteo.com, бесплатно, без ключа): ECMWF ENS 51 членов и др. P(дождь) = доля членов с осадками, полоса P10–P90. Бот precip_soon — порог по P из членов | ⬜ |
| 4. Синхронизация бота + lead-корзины | бот считает те же веса из своего KV (сейчас его AVG — невзвешенный mean, рассинхрон с сайтом); predictions на D+1…D+5 с полем lead, статистика по корзинам D1–2 / D3–5 | ⬜ |
...
**Тест precip-метрик:** `node scripts/test-precip-metrics.mjs` — извлекает реальные функции из app.js, 12 синтетических ассертов + прогон на живых данных Worker'а (Харьков + Південне).
