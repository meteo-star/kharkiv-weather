...
> **Текущая версия:** v1.63.0 (автокалибровка «дождь/сухо»: порог мм для плитки подбирается по реальным замерам точки с предохранителями 60/25/10пп; строка в accuracy-таблице; на 13.08 — «копится 56/60, 24/25», активируется сама. Бэктест: порог 0.3 даёт 23% промахов vs 40% у 0.5. v1.62.x ниже)
