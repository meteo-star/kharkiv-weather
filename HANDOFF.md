...
> **Текущая версия:** v1.59.0 (**«Сегодня» = остаток суток везде**: `dayPrecipView()` в app.js для плитки дня и мини-метрики модалки, `sEff = max(s, nowIdx)` в утренней сводке бота. Дневная вероятность = max почасовых (было mean-of-max → завышало и расходилось с часами). «Шанс» ловит ливень, который видит меньшинство моделей (`pmmMax`/`pmmPoss`), т.к. медиана Smart его обнуляет. Порог `PRECIP_POP_SHOW` 30%→20%. Ключи `precip.restOfDay/restOfDayShort/upTo` ×16 языков)
