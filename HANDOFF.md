...
> **Текущая версия:** v1.50.3 (precip cooldown 6→3ч + smart-reset на конец дождя)
...
- **Деплой и git push — автомат** (изменено 31 мая 2026): после правок Claude сам делает `git commit` → `git push` → `cd bot && npx wrangler deploy` (если менялся `bot/`). `wrangler login` сохранил токен локально, деплой работает из любой сессии. Запрашивать подтверждение не нужно
