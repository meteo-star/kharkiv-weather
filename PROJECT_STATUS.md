...
> **Статус v1.64.1 (лента неба читаемая: высота 11 на 20px, цвета насыщеннее (альфа 0.85-0.95), эмодзи 14px на тёмной подложке-кружке r=11; фидбек «значки не читаются, на телефоне мелко»):** ✅ закрыт
> **Статус v1.64.0 («лента неба» на графике осадков: тонкая градиентная полоса облачности по часам вверху графика (золотистое ясно, голубое переменно, серое пасмурно) + эмодзи на однородных участках от 3 часов + облачность в тултипе; данные h.cl есть во всех источниках; ОТКАТ: флаг PRECIP_CLOUD_BAND=false в renderPrecipChart либо git-тег v1.63.1; изменение аддитивно, существующие слои не тронуты):** ✅ закрыт
> **Статус v1.63.1 (ряд вероятностей на графике осадков: под графиком 48ч печатаются те же h.p проценты, что в списке часов модалки (порог показа 10%, подсветка 20%/60%), вероятность добавлена в тултип; причина: часы говорили процентами, график только миллиметрами, и «73%» рядом с нулевой медианной линией читалось как несоответствие; данные при проверке совпадали один в один):** ✅ закрыт
> **Статус v1.63.0 (автокалибровка порога «дождь/сухо»: computeWetCalibration() подбирает порог мм по накопленным archive-замерам KV (минимизация 1−CSI), предохранители — ≥60 замеров, ≥25 дождей, выигрыш ≥10 п.п.; порог применяется к состоянию «дождь» плитки дня (getWetShowThreshold, дефолт 0.3); строка прогресса/результата в accuracy-таблице ×16; определение события WET_DAY_MM=0.5 и метрики таблицы НЕ тронуты):** ✅ закрыт
> **Статус v1.62.4 (сворачивание hero ещё мягче: 0.55s + decelerate-кривая cubic-bezier(0.25,0.8,0.3,1) — быстрый старт, долгое плавное затухание; «грубость» давал резкий финиш material-кривой):** ✅ закрыт
> **Статус v1.62.3 (сворачивание hero: 0.28s было резковато — возвращены исходные 0.4/0.42s; перф-фиксы v1.62.0 при этом сохранены, рывка нет):** ✅ закрыт
> **Статус v1.62.2 (согласованность сводки бота: заголовок = состояние ТЕКУЩЕГО часа (не daily.wc за сутки), «Дождя не ожидается» не пишется при идущей мороси (block.precip.nowThenDry ×16), «завтра» без оксюморона «Морось, без осадков» (даунгрейд wc при сумме ≤0.5); сайт: «до HH сухо» скрывается при активном nowcast; плавное сворачивание hero ВОЗВРАЩЕНО (0.28s вместо мгновенного скачка)):** ✅ закрыт
> **Статус v1.62.1 (скролл ещё плавнее: blur 14→10 на десктопе, content-visibility:auto для нижних карточек (pollen/climate/astro), hover-freeze строк при прокрутке; точность: сводка не пишет «возможен дождь с {уже текущего часа}», минутный тик пересчитывает «остаток дня» при смене календарного часа на открытой странице):** ✅ закрыт
> **Статус v1.62.0 (полный аудит точности Smart: полоса ансамбля в модалке «Сегодня» = остаток суток (modal.precipBandRest ×16); PERF плавного скролла: снят глобальный scroll-behavior:smooth, blur 24→14 (моб. 9), у строк дней свой backdrop-filter отключён, sticky-hero без layout-transition, пауза декоративных анимаций при прокрутке, лёгкий hover строк, чипы без transition:all):** ✅ закрыт
> **Статус v1.61.0 (Smart и осадки: вероятность часа = max(ансамбль ECMWF, доля из 8 моделей) — раньше при сухом ECMWF Smart показывал 0–2% там, где UKMO/GFS дают 2 мм; дневная сумма для «Сегодня» распределяется по временнóму профилю (сумма почасовых МЕДИАН систематически = 0); сводка называет возможное количество вместо «слабых осадков» — outlook.maybeFrom/maybeAmount ×16):** ✅ закрыт
> **Статус v1.60.0 (морось перестала пропадать: порог «шанса» 0.3→0.1 мм в часах и 0.5→0.2 в дне, полоса графика привязана к количеству, а не к вероятности; сводка не пишет «осадков не ожидается», когда nowcast видит морось — ключ outlook.maybePrecip ×16):** ✅ закрыт
> **Статус v1.59.0 (КРИТИЧНО для точности: «Сегодня» = ОСТАТОК суток везде — плитка дня, мини-метрика модалки и утренняя сводка бота больше не считают прошедшие часы; дневная вероятность = max почасовых вместо mean-of-max; «шанс» ловит ливень, который видит меньшинство моделей (медиана Smart его обнуляла); порог 30%→20%; ×16 языков):** ✅ закрыт
