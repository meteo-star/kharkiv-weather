...
> **Текущая версия:** v1.15-mobile-safe-area
...
> **Статус фазы Л (Mobile safe-area + swipe):** ✅ закрыта (v1.15)
...
✅ **Фаза Л закрыта (Л1).** Mobile safe-area — все модалки корректно отступают от iOS status bar / Dynamic Island / Home Indicator + swipe-down закрытие на ≤640px + надёжное центрирование текущего часа.
...
*Документ обновлён: 16 мая 2026 после релиза v1.15-mobile-safe-area (фаза Л: критический фикс iOS-обрезки модалок status bar'ом + swipe-down закрытие на мобильных + надёжное центрирование текущего часа в hourlyDetailModal).*
*Следующий шаг: Гипертермия/гипотермия алерты / .ics-экспорт календаря / Sticky-табы метрик / Темы (светлая/тёмная) / Push-уведомления (требует backend) — выбор за пользователем.*
