...
> **Текущая версия:** v1.14-realistic-hero
...
*Документ обновлён: 16 мая 2026 после релиза v1.14-realistic-hero (фаза К: фотореалистичная hero-сцена с 8 фото-фонами Unsplash, анимированные частицы дождя/снега/тумана/молний с реакцией на интенсивность из API, синхронизация далёких молний со storm-heatmap).*
*Следующий шаг: Гипертермия/гипотермия алерты / .ics-экспорт календаря / Sticky-табы метрик / Swipe-down закрытие модалок / Темы (светлая/тёмная) / Push-уведомления (требует backend) — выбор за пользователем.*
