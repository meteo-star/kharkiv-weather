...
> **Текущая версия:** v1.15.6-hour-center-full
...
> **Статус фазы Л (Mobile safe-area + swipe + iOS PWA):** 🟡 в работе (v1.15 → v1.15.6)
...
🟡 **Статус: почти готова, остался 1 баг.** Большинство iPhone-проблем решено за 6 итераций. Незакрытое: центрирование текущего часа в **полной** модалке «Сегодня — почасовый» (где 11 SVG-чартов) на iPhone PWA. В метрических модалках (Ветер / Давление / Влажность с 1-2 чартами) — работает.
...
| Л1.6 | `v1.15.6-hour-center-full` | **Попытка зафиксить центрирование в полной модалке (11 метрик).** В метрических работало, в полной — нет, т.к. 11 SVG-чартов рендерятся дольше и layout не успевает стабилизироваться за 1000мс. Добавлены событийные триггеры: (1) `transitionend` на `.hdm-sheet` — точный момент завершения анимации открытия; (2) `ResizeObserver` на `hdm-scroll` — ловит каждый ресайз во время рендера чартов, после 8 событий или 1.8s отключается; (3) Дополнительный `setTimeout(1500ms)` для очень медленных устройств. Функция `positionToHour` идемпотентна — повторные вызовы безвредны. **Пользователь поставил проект на паузу до проверки итерации.** Возможно потребуется ещё одна итерация v1.15.7 если этого не хватит. |

#### Технический контекст для будущей итерации

**Если v1.15.6 не центрирует час в полной модалке — следующие гипотезы:**
1. **DOM ещё не готов даже после `transitionend`.** Можно попробовать `setTimeout(positionToHour, 2500)` как крайнюю меру.
2. **`scrollEl.scrollLeft` сбрасывается каким-то поздним reflow.** Можно добавить `IntersectionObserver` на target-ячейку — детектировать когда она вообще не в viewport, и в этот момент скроллить.
3. **Проблема в `<svg>`-чартах** — может, они задают `width:100%` и постепенно «разъезжаются», меняя scrollWidth. Можно дать им фиксированную ширину при render.
4. **`-webkit-overflow-scrolling: touch` ВСЁ ЕЩЁ блокирует** — несмотря на временный сброс. Попробовать вообще убрать это свойство из CSS (на iOS 13+ momentum scroll и так работает по дефолту с `overflow: auto`).
5. **Радикальный путь:** при открытии модалки делать `scrollEl.style.scrollSnapType = 'x mandatory'` + поставить `scroll-snap-align: center` на now-cell — браузер сам зафиксирует центрирование. Минус: ломает swipe-down закрытие и плавный horizontal scroll.

**Архитектурное место:** функция `openHourlyDetail` в `index.html` около строки 4257. Вся логика центрирования в одном месте внутри неё (helper `positionToHour` + триггеры).

**Как тестировать:** открыть PWA на iPhone → main screen → клик на любой час в почасовой ленте (НЕ на метрические плитки) → должна открыться модалка с 11 чартами + sticky-часы сверху → текущий час должен быть в центре экрана. NOW_HOUR определяется через `new Date().getHours()` в init.
...
🟡 **Фаза Л в работе (Л1.0 → Л1.6).** Mobile safe-area, swipe-down, iOS PWA — большинство фиксов внедрено. **Остался 1 баг:** центрирование текущего часа в полной модалке «Сегодня — почасовый» (11 метрик) на iPhone PWA не работает (в метрических модалках с 1-2 чартами — работает).

**При возобновлении проекта — продолжаем с фазы Л1.7** (см. «Технический контекст для будущей итерации» в фазе Л выше). Либо переходим к новой фазе из бэклога (часть 7).
...
*Документ обновлён: 16 мая 2026 после серии релизов v1.15 → v1.15.6 (фаза Л: 6 итераций iOS-фиксов — safe-area для модалок и главного экрана, swipe-down закрытие, PWA status-bar, network-first SW для мгновенных обновлений, центрирование часа через scrollIntoView + ResizeObserver). **Пользователь поставил проект на паузу с открытым вопросом** — центрирование часа в полной модалке с 11 метриками на iPhone PWA после v1.15.6 нуждается в проверке; если не работает — см. «Технический контекст для будущей итерации» в фазе Л.*

*Бэклог следующих шагов (выбирается пользователем при возобновлении): закрыть фазу Л через Л1.7 / Алерты гипертермии-гипотермии / .ics-экспорт календаря / Sticky-табы метрик / Светлая тема + автопереключение / Сравнение городов split-view / Виджет «Лучший день недели для X» / Push-уведомления (требует backend через OneSignal или Cloudflare Worker).*
